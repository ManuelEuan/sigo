                            ...
                            <div class="card-body">
                                ...
                                <div class="row text-center mb-2">
                                    <div class="col-4">
                                        ...
                                        <span class="font-14">PAT</span></div>
                                    ...
                                    <div class="col-4"><h4 class="m-b-0" style="font-weight:700"><?=${'eje_'.$d->iIdEje}['entregables']?> </h4>
                                        <span class="font-14">Entregables</span></div>
                                </div>
...
<section>
    ...
    <div class="row mb-4">
        ...
        <div class="col-6">
            <label>Filtrar por entregable:</label>
...
<section>
    ...
    {
        $data_comp_est = '';
        $numcomp = $avance = $total = 0;
        $html = '';
        foreach ($comp_est as $row) 
        {
            $numcomp += $row->numcomp;
            $avance += $row->avance;
            $porcentajeAvance = ($row->numcomp > 0) ? round($row->avance/$row->numcomp):0;
    ...
    selected: true, avance:$porcentajeAvance}":"{name:'$row->vEstatus',color: \"$row->vColor\",y:$row->numcomp, avance:$porcentajeAvance}";
            $html.= '<div class="col-4 text-center"><h4 style="color:'.$row->vColor.';font-weight:700;">'.$row->numcomp.'</h4><h5>'.$estatus.'</h5></div>';
            $total+= $row->numcomp;
        }
        $porcentajeAvance = ($numcomp > 0) ? round($avance/$numcomp):0;
            ...
            <div class="col-md-12 text-left">            
                <div class="card" style="background-color: #FFFFFF !important;">
                    <div class="card-body">
                        ...
                        <div class="row">
                                    <div class="col-12 text-center"><h5><span style="font-weight:700;">'.$porcentajeAvance.' %</span> Avance</h5></div>
                                </div>  
                        <div class="row">
                            <div class="col-md-12" id="pie-compromisos" class="m-t-30" style="min-height:250px; width:100%;">
                            </div>
                        </div>
    ...
    <div class="row">
        ...
        foreach ($comp as $c)
        {
            ...
            {   
                $numcomp += $est->numcomp;
                $avance += $est->avance;
                $porcentajeAvance = ($est->numcomp > 0) ? round($est->avance/$est->numcomp):0;
    ...
    selected:true, avance:$porcentajeAvance}":"{name:'$est->vEstatus',color: \"$est->vColor\",y:$est->numcomp, avance:$porcentajeAvance}";
                $htmTotales.='<div class="col-4"><h4 style="color:'.$est->vColor.';font-weight:700;">'.$est->numcomp.'</h4><h5>'.$estatus.'</h5></div>';
                $total+= $est->numcomp;
            }
            $porcentajeAvance = ($numcomp > 0) ? round($avance/$numcomp):0;
        ?>
            <div <?php echo ($all_sec > 0) ?  'class="col-md-4"':'class="col-md-12"' ?>>
                <div class="card" style="background-color: #FFFFFF !important;">
                        <div class="card-body">
                            <div class="row">
                                <div class="col-2 circle text-center"><img src="<?=$c->vIcono?>" width="200px;" style="background:#<?=$c->vColorDesca?>;"></div>
                                <div class="col-10 text-center">
                                    <h4 style="color:#<?=$c->vColorDesca?>;font-weight: bold;"><?=$c->vEje?></h4>
                                </div>
                            </div>
                            
                            <div class="row">
                                <div class="col-12 text-center"><h4><span style="font-weight:700;"><?=$total;?></span> Compromisos</h4></div>
                            </div>
                            <div class="row">
                                <div class="col-12 text-center"><h5><span style="font-weight:700;"><?=$porcentajeAvance;?>%</span> Avance</h5></div>
                            </div>                            
