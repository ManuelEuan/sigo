...
class M_dash extends CI_Model {
	...
	public function retos(){
		$datos = '';
		$datos = array();
		$this->db->select('iIdReto, vDescripcion');
		$this->db->from('Reto');
	...
	public function avance_anio_eje($anio,$eje = 0){
		...
		{
			$sql = 'SELECT tt."iIdEje", count(tt."iIdDetalleActividad") AS numact, sum(tt.navanceactividad) AS avance_r
				FROM (	SELECT DISTINCT t."iIdEje



				", t."iIdDetalleActividad", t.navanceactividad
						FROM actividades_eje2 t
	...
	{
		$sql = 'SELECT dep."iIdDependencia", dep."vNombreCorto", dep."vDependencia", COUNT(act."vActividad") numact, SUM(dat."nAvance") sumavance, SUM(dat."nPresupuestoAutorizado") autorizado, SUM(av."nEjercido") ejercido
				...
				LEFT JOIN "DetalleEntregable" dent ON dent."iIdDetalleActividad" = dat."iIdDetalleActividad"
				LEFT JOIN "Avance" av ON av."iIdDetalleEntregable" = dent."iIdDetalleEntregable" AND av."iActivo" = 1
	...
	{
		$sql = 'SELECT act."iIdActividad", act."vActividad", dat."nAvance"
				FROM "Dependencia" dep
				INNER JOIN "Actividad" act ON act."iIdDependencia" = dep."iIdDependencia"
