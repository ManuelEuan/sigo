...
class M_reporteCombinado extends CI_Model
{
  ...
  public function reporteHija($idactividad)
  {
    ...
    if (!empty($idactividad)) {
      ...
      $select = 'select idact, nivel, resumennarrativo,tipo, dimension, accion, clave, indicador, meta, frecuencia, operacion, vvariable, unidadmedida, formula, umedioverifica,
      sum(case when (fecha=1) then avance else 0 end) as Enero,
        sum(case when (fecha=2) then avance else 0 end) as Febrero,
        sum(case when (fecha=3) then avance else 0 end) as Marzo,
        sum(case when (fecha=4) then avance else 0 end) as Abril,
        sum(case when (fecha=5) then avance else 0 end) as Mayo,
        sum(case when (fecha=6) then avance else 0 end) as Junio,
        sum(case when (fecha=7) then avance else 0 end) as Julio,
        sum(case when (fecha=8) then avance else 0 end) as Agosto,
        sum(case when (fecha=9) then avance else 0 end) as Septiembre,
        sum(case when (fecha=10) then avance else 0 end) as Octubre,
        sum(case when (fecha=11) then avance else 0 end) as Noviembre,
        sum(case when (fecha=12) then avance else 0 end) as Diciembre
      from
